...
function KidsCategory() {
  ...
  const toggleLike = (id) => {
    if (likedItems[id]) {
      ...
      const likedProduct = kids[category].find((item) => item.id === id);
      if (likedProduct) {
        addToWish(
          likedProduct.id,
          likedProduct.thumb,
  ...
  const toggleCart = (id) => {
    if (addedItems[id]) {
      ...
      const addedProduct = kids[category].find((item) => item.id === id);
      if (addedProduct) {
        addToBasket(
          addedProduct.id,
          addedProduct.thumb,
  ...
  return (
    <div className="bookshop">
      {category &&
        kids[category].map((item) => {
          return (
            <Link key={item.id} className="books-card" to={`/kids/${category}/${item.id}`}>
