...
function WCategory() {
  ...
  const { categoryw } = useParams();
  const toggleLike = (id) => {
    if (likedItems[id]) {
      deleteFromWish(id);
    } else {
      const likedProduct = womens[categoryw].find((item) => item.id === id);
      if (likedProduct) {
        addToWish(
          likedProduct.id,
          likedProduct.thumb,
  ...
  const toggleCart = (id) => {
    if (addedItems[id]) {
      ...
      const addedProduct = womens[categoryw].find((item) => item.id === id);
      if (addedProduct) {
        addToBasket(
          addedProduct.id,
          addedProduct.thumb,
  ...
  return (
    <div className="bookshop">
      {categoryw &&
        womens[categoryw].map((item) => {
          return (
            <Link key={item.id} className="books-card" to={`/women/${categoryw}/${item.id}`}>
