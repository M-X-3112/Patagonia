...
function BestSeller() {
  ...
  const toggleLike = (id) => {
    if (likedItems[id]) {
      ...
      const likedProduct = best.find((item) => item.id === id);
      if (likedProduct) {
        addToWish(
          likedProduct.id,
          likedProduct.thumb,
  ...
  const toggleCart = (id) => {
    if (addedItems[id]) {
      ...
      const addedProduct = best.find((item) => item.id === id);
      if (addedProduct) {
        addToBasket(
          addedProduct.id,
          addedProduct.thumb,
  ...
  return (
    <div className="books-main">
      ...
      <div className="bookshop">
        {best.map((item) => (
          <Link key={item.id} className="books-card" to={`/bestSeller/${item.id}`}>
            <div className="book-bg"></div>
            <span id="card-like-change"
              onClick={(e) => {
                e.preventDefault();
                toggleLike(item.id);
              }}
