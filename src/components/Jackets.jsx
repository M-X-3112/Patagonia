...
function Jackets() {
  ...
  const toggleLike = (id) => {
    if (likedItems[id]) {
      ...
      const likedProduct = jackets.find((item) => item.id === id);
      if (likedProduct) {
        addToWish(
          likedProduct.id,
          likedProduct.thumb,
  ...
  const toggleCart = (id) => {
    if (addedItems[id]) {
      ...
      const addedProduct = jackets.find((item) => item.id === id);
      if (addedProduct) {
        addToBasket(
          addedProduct.id,
          addedProduct.thumb,
  ...
  return (
    <div className="books-main">
      ...
      <div className="bookshop">
        {jackets.map((item) => (
          <Link key={item.id} className="books-card" to={`/jackets/${item.id}`}>
